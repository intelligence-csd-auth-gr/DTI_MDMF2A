...
class MDMFAUPR(MultiViewInductiveModelBase):
    ...
    def _compute_loss(self):
        ...
        # r_ds = []
        for i in range(self._n_dsims):
            # r_ds.append(self._ad[i]*np.trace(self._U.T@self._DLs[i]@self._U))
            r_d += self._ad[i]*np.trace(self._U.T@self._DLs[i]@self._U)#/self._n_drugs
        r_d *= 0.5*self.lambda_d
        r_t = 0
        # r_ts = []
        for i in range(self._n_tsims):
            # r_ts.append(self._at[i]*np.trace(self._V.T@self._TLs[i]@self._V))
